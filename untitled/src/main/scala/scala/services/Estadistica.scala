...
object Estadistica{
  def promedio(temperaturas:List[Estudiante]): Double = {
    temperaturas.map(_.temp_tarde).sum / temperaturas.length
  }

  def maxima_temperatura(temperaturas:List[Estudiante]): Double = {
    temperaturas.map(_.temp_tarde).max
  }

  def desviacion_estandar(temperaturas:List[Estudiante]): Double = {
